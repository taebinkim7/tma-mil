...
if __name__ == '__main__':
    ...
    parser.add_argument('--instance-sizes', nargs='+', help='instance sizes' )
    parser.add_argument('--instance-strides', nargs='+', help='instance strides' )
    ...
    instance_strides = args.instance_strides
    if len(instance_strides) == 1:
        instance_strides = len(instance_sizes) * instance_strides
    ...
    if len(instance_sizes) != len(instance_strides):
        print('Error: number of instance sizes and number of instance strides do not match')
        sys.exit(1)
    ...
    # read in multi-scale CNN features
    ms_feats = {}
    for instance_size, instance_stride in zip(instance_sizes, instance_strides):
    ...
    for c,cat_name in enumerate(cats):
        ...
        for f,(idx_train,idx_test) in enumerate(idx_train_test):
            ...
            # concatenate quantile function vectors
            Q_train = []
            Q_test = []
            for instance_size in instance_sizes:
                X_train = [ms_feats[instance_size][samples[i]] for i in idx_train if samples[i] in ms_feats[instance_size]]
                X_test = [ms_feats[instance_size][samples[i]] for i in idx_test if samples[i] in ms_feats[instance_size]]

                assert mi_type == 'quantile', 'Quantile function must be used for multi-scale MI learning.'
                if quantiles is not None:
                    options['quantiles'] = int(quantiles)

                # train instance-level classifier
                ...
                if sample_weight is not None:
                    ...
                    sw = None
                model.fit( X_train, y_train, calibrate=calibrate, param_search=True, sample_weight=sw, early_stop=True )

                Q_train.append(model.get_qf(X_train))
                Q_test.append(model.get_qf(X_test))

            Q_train = np.hstack(Q_train)
            Q_test = np.hstack(Q_test)

            # train bag-level classifier using concatenated quantile function vectors
            model_agg = LinearClassifier(classifier=agg_type)
            C_agg, gamma_agg = model_agg.param_search(Q_train, y_train, sample_weight=sw, quick=False)
            model_agg.C = C_agg
            model_agg.fit(Q_train, y_train, sample_weight=sw, calibrate=calibrate)

            p_predict = model_agg.predict( Q_test )
