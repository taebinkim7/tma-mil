...
def cal_scores(X, y, level=level):
    if level == 'instance':
...
for i in range(k):
    ...
    # clf_scores, _, instance_y_test = cal_scores(X_test, y_test, 'instance')
    ...
    # compute scores
    clf_scores, opc1_scores, _ = cal_scores(X_test, y_test, 'bag')
    ...
    # clf_scores, _, y_test = cal_scores(X_test, y_test, 'bag')
    # pos_idx = (y_test == 1)
    # neg_idx = (y_test == 0)
    n = len(y_test)
    noise = (np.arange(n) - n // 2) / (3 * n)
