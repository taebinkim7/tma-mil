...
class MSSIL(LinearClassifier):
    """Multi-scale Single-Instance Learning applied to MI data."""

    def __init__(self, classifier='svm', kernel='linear', C=None, p=3, gamma=1e0, T=None, predict_type='median', class_weight='balanced', calibrate=False, subset=None, quantiles=16, metric='acc', n_jobs=1, verbose=True, agg_type='svm'):
    ...
    def fit(self, bags, y=None, sample_weight=None, param_search=False, calibrate=False):
        ...
        super(SIL, self).fit(svm_X, svm_y, sample_weight=sample_weight2, calibrate=((param_search and self.calibrate) or calibrate), cv_split=cv_split_inst)

        if 'quantile' in self.predict_type:
            self.train_model_agg( bags, y, cv_split_bags=cv_split_bags, sample_weight=sample_weight, classifier=self.agg_type )
        else:
            self._model_agg = None

        return self

    def train_model_agg( self, bags, y, cv_split_bags=None, sample_weight=None, param_search=True, classifier='svm' ):
        ...
        model_agg.fit( p, test_y, sample_weight=sample_weight, param_search=param_search, calibrate=self.calibrate )
