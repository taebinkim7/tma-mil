...
class SIL(LinearClassifier):
    ...
    def __init__(self, classifier='svm', kernel='linear', C=None, p=3, gamma=1e0, T=None, predict_type='median',
                 class_weight='balanced', calibrate=False, subset=None, quantiles=16, metric='acc', n_jobs=1,
                 verbose=True, agg_type='svm', n_components=None):
        ...
        self._pc_loadings = None
        self._opc1 = None
        self._opc2 = None
        self.n_components = n_components
        super(SIL, self).__init__(classifier=classifier, kernel=kernel, class_weight=class_weight, C=C, p=p,
                                  gamma=gamma, subset=subset, metric=metric, n_jobs=n_jobs, verbose=verbose)

    def fit(self, bags, y=None, sample_weight=None, param_search=False, calibrate=False, early_stop=False):
        """Fit model."""
        ...
        if sample_weight is not None:
            ...
            sample_weight2 = np.hstack(np.array([[1.0/len(bag)*sw]*len(bag)
                                                 for bag, sw in zip(bags, sample_weight)])).squeeze()
        ...
        super(SIL, self).fit(svm_X, svm_y, sample_weight=sample_weight2,
                             calibrate=((param_search and self.calibrate) or calibrate), cv_split=cv_split_inst)

        # stop early without training bag-level classifier
        if early_stop:
            return self

        if 'quantile' in self.predict_type:
            self.train_model_agg(bags, y, cv_split_bags=cv_split_bags, sample_weight=sample_weight,
                                 classifier=self.agg_type, predict_type=self.predict_type)
        else:
            self._model_agg = None

        return self

    def get_qf(self, bags):
        """Calculate quantile functions."""
        ...
        for i in range(len(bags)):
            pi = super(SIL, self).predict(bags[i])
            if pi.shape[1] == 2:
    ...
    def train_model_agg(self, bags, y, cv_split_bags=None, sample_weight=None, classifier='svm',
        ...
        if 'pca' in predict_type:
            ...
            if self.n_components is not None:
                inst_pca = PCA(n_components=self.n_components).fit(svm_X)
            else:
                inst_pca = PCA().fit(svm_X)
            self._pc_loadings = inst_pca.components_
        if 'opc' in predict_type:
            model = self._model
            svm_X = np.vstack(self._bags)
            # svm_scores = svm_X @ model.coef_.T + model.intercept_
            # svm_scores = svm_scores.reshape(-1)
            # calculate orthogonal PC scores
            X1 = svm_X - svm_X @ model.coef_.T @ model.coef_ / (model.coef_ @ model.coef_.T)
            pca = PCA().fit(X1)
            self._opc1 = pca.components_[0]
            self._opc2 = pca.components_[1]
            # opc1_scores = svm_X @ opc1
            # opc2_scores = svm_X @ opc2
        ...
        for f in range(5):
            ...
            for i in test_idx:
                ...
                if 'pca' in predict_type:
                    ...
                    pc_dir_vars = np.var(pc_scores, axis=0)
                    q = np.concatenate([q, pc_dir_vars])
                if 'opc' in predict_type:
                    opc1_scores = bags[i] @ self._opc1.T
                    opc2_scores = bags[i] @ self._opc2.T
                    # q = np.concatenate([q, opc1_scores])
                    opc1_quantiles = np.percentile(opc1_scores, quantiles).reshape(-1)
                    opc2_quantiles = np.percentile(opc2_scores, quantiles).reshape(-1)
                    q = np.concatenate([q, opc1_quantiles, opc2_quantiles])
                p.append(q)
                test_y.append(y[i])
        p = np.vstack(p)
        ...
        # train model
        model_agg = LinearClassifier(classifier=classifier)
        self.C_agg, self.gamma_agg = model_agg.param_search(p, test_y, sample_weight=sample_weight, quick=False)
        model_agg.C = self.C_agg
        model_agg.fit(p, test_y, sample_weight=sample_weight, calibrate=self.calibrate)
    ...
    def predict(self, bags, y=None):
        ...
        try:
            p = _inst_to_bag_preds(inst_preds, bags, self.predict_type, self.T, self._model_agg, self._pc_loadings,
                                   self._opc1, self._opc2)
    ...
    def score(self, bags, y=None, sample_weight=None):
        ...
        if self.metric == 'roc_auc':
            ...
            a = sklearn.metrics.balanced_accuracy_score(y, d, sample_weight=sample_weight)
    ...
    def param_search(self, bags, y, instances, classes, quick=True, C=1.0, gamma=1.0, bag_inst_idx=None,
                     sample_weight=None, inst_search=False):
        ...
        while True:
            ...
            if inst_search:
                ...
                bestC, bestg = model.param_search(instances, classes, quick, C=C, gamma=gamma,
                ...
                est = SIL(classifier=self.classifier, kernel=self.kernel, predict_type=self.predict_type,
                          class_weight=self.class_weight, p=self.p, subset=self.subset, quantiles=self.quantiles,
                          metric=self.metric)
                gridcv = sklearn.model_selection.GridSearchCV(est, [{'C': Cvals2,'gamma': gvals}], cv=skf,
                                                              n_jobs=self.n_jobs, refit=False)
                gridcv.fit(bags, y, sample_weight=sample_weight, calibrate=self.calibrate)
...
def _inst_to_bag_preds(inst_preds, bags, predict_type='median', T=None, model_agg=None, pc_loadings=None, opc1=None, opc2=None):
    ...
    if predict_type == 'quantile':
        ...
        return r.squeeze()
    elif predict_type == 'quantile_mean':
        if inst_preds.shape[1] == 2:
            r = np.array([model_agg[0].predict(np.concatenate([np.percentile(inst_preds[slice(*bidx),1], model_agg[1]).reshape(1,-1), np.mean(bag, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        else:
            r = np.array([model_agg[0].predict(np.concatenate([np.array(np.hstack([ np.percentile(inst_preds[slice(*bidx),c], model_agg[1]) for c in range(inst_preds.shape[1])]).reshape((1,-1))).flatten(),
                                                               np.mean(bag, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        return r.squeeze()
    elif predict_type == 'quantile_pca':
        if inst_preds.shape[1] == 2:
            r = np.array([model_agg[0].predict(np.concatenate([np.percentile(inst_preds[slice(*bidx),1], model_agg[1]).reshape(1,-1), np.var(bag @ pc_loadings.T, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        else:
            r = np.array([model_agg[0].predict(np.concatenate([np.array(np.hstack([ np.percentile(inst_preds[slice(*bidx),c], model_agg[1]) for c in range(inst_preds.shape[1])]).reshape((1,-1))).flatten(),
                                                               np.var(bag @ pc_loadings.T, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        return r.squeeze()
    elif predict_type == 'quantile_mean_pca':
        if inst_preds.shape[1] == 2:
            r = np.array([model_agg[0].predict(np.concatenate([np.percentile(inst_preds[slice(*bidx),1], model_agg[1]).reshape(1,-1), np.mean(bag, axis=0), np.var(bag @ pc_loadings.T, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        else:
            r = np.array([model_agg[0].predict(np.concatenate([np.array(np.hstack([ np.percentile(inst_preds[slice(*bidx),c], model_agg[1]) for c in range(inst_preds.shape[1])]).reshape((1,-1))).flatten(),
                                                               np.mean(bag, axis=0), np.var(bag @ pc_loadings.T, axis=0)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        return r.squeeze()
    elif predict_type == 'quantile_opc':
        if inst_preds.shape[1] == 2:
            r = np.array([model_agg[0].predict(np.concatenate([np.percentile(inst_preds[slice(*bidx),1], model_agg[1]).reshape(1,-1),
                                                               np.percentile(bag @ opc1.T, model_agg[1]).reshape(1,-1),
                                                               np.percentile(bag @ opc2.T, model_agg[1]).reshape(1,-1)], axis=1))
                          for bidx, bag in zip(slices(map(len, bags)), bags)])
        else:
            r = np.array([model_agg[0].predict(np.concatenate([np.array(np.hstack([ np.percentile(inst_preds[slice(*bidx),c], model_agg[1]) for c in range(inst_preds.shape[1])]).reshape((1,-1))).flatten(),
                                                               np.percentile(bag @ opc1.T, model_agg[1]).reshape(1,-1),
        ...
        print('Error: aggregation method %s not supported'%predict_type)
        sys.exit(1)
